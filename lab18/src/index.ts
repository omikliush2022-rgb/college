...
import { TodoRepository, ApiTodoRepository } from "./repositories/TodoRepository";
...
(el.querySelector(".task-card__id") as HTMLElement).textContent =
String(task.id);
(el.querySelector(".task-card__userid") as HTMLElement).textContent =
String(task.userId);
(el.querySelector(".task-card__title") as HTMLElement).textContent =
task.title;
(el.querySelector(".task-card__tag") as HTMLElement).textContent =
task.tag;
(el.querySelector(".task-card__desc") as HTMLElement).textContent =
task.description;
(el.querySelector(".task-card__deadline") as HTMLElement).textContent =
formatDate(task.deadline);
(el.querySelector(".task-card__created") as HTMLElement).textContent =
formatDate(task.createdAt);
if (task.isDone) {
(el.querySelector(".task-card__status") as HTMLElement).textContent =
`✓ Виконано`;
(el.querySelector(".btn--complete") as HTMLElement).style.opacity =
"0.5";
}
...
(form.elements.namedItem("description") as
HTMLInputElement).value = (card.querySelector(".task-card__desc") as
HTMLElement).textContent || "";

(form.elements.namedItem("tag") as HTMLInputElement).value =
(card.querySelector(".task-card__tag") as HTMLElement).textContent || "";
let deadline = (card.querySelector(".task-card__deadline") as

HTMLElement).textContent;

(form.elements.namedItem("deadline") as
HTMLInputElement).value = deadline === "Без дати" ? "" : (deadline ||
"");
